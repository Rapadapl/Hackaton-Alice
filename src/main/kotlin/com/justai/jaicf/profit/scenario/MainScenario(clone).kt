...
object `MainScenario(clone)` : Scenario() {
    init {
        state("main") {
            activators {
            ...
            action {
                reactions.say(
                    "Я подскажу, какой товар выгоднее. " +
            ...
            action {
                activator.alice?.run {
                    ...
                    context.session["first"] = Product(
                        firstAmount?.value?.toAmount() ?: 1.0,
                        firstPrice!!.value.int,
                        firstUnit?.value?.content ?: ""
                                                      )
                    context.session["second"] = secondPrice?.let {
                        Product(
                            secondAmount?.value?.toAmount() ?: 1.0,
                            secondPrice.value.int,
                            secondUnit?.value?.content ?: ""
                               )
                    }
